...
class Web_model extends CI_Model
{
  ...
  public function getprodukperkat($id,$limit,$start)
  {
    $this->db->select('*');
    $this->db->join('tbl_kategori', 'tbl_kategori.id = tbl_produk.kategori', 'left');
    $this->db->where('id', $id);
  ...
  public function pengrajinId($id)
  {
    return $this->db->get_where('tbl_pengrajin', ['id' => $id])->row_array();
  }
  public function simpan_buatfurniture()
  {
    $config['upload_path'] = './assets/images/';
    $config['allowed_types'] = 'jpg|png|gif|jpeg';
    $config['max_size'] = '2048000';
    ...
    $data = array(
      'id'=> "",
      'pengrajin'=> $this->input->post('pengrajin'),
      'nama'=> $this->input->post('nama'),
      'email'=> $this->input->post('email'),
    ...
    $this->db->insert('tbl_buatfurniture', $data);
    $this->session->set_flashdata('pesan', 'Ditambahkan');
    redirect('website/pengrajin');
  }
}
